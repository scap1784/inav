	...
	while (1==1) {
		...
		for (loop=0; loop<max_clients; loop++) {
			if (FD_ISSET(client_socket[loop], &readfds)) {
				...
				/* 
				 *  * use read() to read from the socket into a buffer
				 *  using something
				 *   * similar to the following:
				 *    *
				 *     * If the read didn't cause an error then send
				 *     buffer to all 
				 *      * client sockets in the array - use for loop
				 *      and send() just
				 *       * as if you were sending it to one connection
				 *        *
				 *         * important point to note is that if the
				 *         connection is char-by-char
				 *          * the person will not be able to send a
				 *          complete string and you will
				 *           * need to use buffers for each
				 *           connection, checking for overflows and
				 *            * new line characters (\n or \r)
				 *             */
			}
		}
